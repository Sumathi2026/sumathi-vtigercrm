...
<form id="headerForm" method="POST">
            ...
            <div class="info-row row headerAjaxEdit td">
                <div class="col-lg-7 fieldLabel">
                    ...
                    {if $FIELD_MODEL->isEditable() eq 'true' && $LIST_PREVIEW neq 'true' && $IS_AJAX_ENABLED eq 'true'}
                        ...
                        <span class="action">
                         <a href="index.php?module={$MODULE_NAME}&view=Edit&record={$RECORD->getId()}" class="fa fa-pencil" title="Edit"></a>
                       </span>
                    {/if}
                </div>
            </div>
    ...
    {* ✅ Updated to use new field name cf_915 *}
    {assign var=APPROVAL_STATUS value=$RECORD->get('cf_915')}
    <div class="info-row row headerAjaxEdit td">
        <div class="col-lg-7 fieldLabel">
            <span class="value">
              <b>Approval Status : </b> {$APPROVAL_STATUS|default:"Pending"}
            </span>
        </div>
    </div>

...
Vtiger_Detail_Js("Potentials_Detail_Js",{
	...
	/**
	 * =============================
	 * NEW Approve / Reject Functions
	 * =============================
	 */

	// Custom Approve / Reject function
	approveOrReject: function (url, acpStatus) {
		var self = this;

		// If Rejected, confirm before continuing
		if (acpStatus === 'Rejected') {
			var confMsg = "Do you really want to Reject this record?";
			app.helper.showConfirmationBox({ 'message': confMsg }).then(function () {
				self.sendApprovalRequest(url, acpStatus);
			});
		} else {
			self.sendApprovalRequest(url, acpStatus);
		}
	...
	sendApprovalRequest: function (url, acpStatus) {
		app.helper.showProgress();

		app.request.post({
			url: url,
			data: { status: acpStatus }
		}).then(
			function (err, data) {
				app.helper.hideProgress();
				if (err === null) {
					app.helper.showSuccessNotification({
						message: "Record " + acpStatus + " successfully."
					});

<?php
class Guard_DetailView_Model extends Vtiger_DetailView_Model {
	...
	public function getDetailViewLinks($linkParams) {
		...
		$recordModel = $this->getRecord();
		$recordId = $recordModel->getId();
		$acceptStatus = $recordModel->get('cf_915');

		// Add Approve/Reject only if not yet approved or rejected
		if ($acceptStatus !== 'Approved' && $acceptStatus !== 'Rejected') {
			$approveLink = array(
				'linktype' => 'DETAILVIEWBASIC',
				'linklabel' => 'Approve',
				'linkurl' => "javascript:Guard_Detail_Js.approveOrReject('index.php?module=" . $this->getModule()->getName() .
					"&action=ApproveOrReject&record=$recordId&source_module=Guard','Approved')",
				'linkicon' => ''
			);
			$linkModelList['DETAILVIEWBASIC'][] = Vtiger_Link_Model::getInstanceFromValues($approveLink);

			$rejectLink = array(
				'linktype' => 'DETAILVIEWBASIC',
				'linklabel' => 'Reject',
				'linkurl' => "javascript:Guard_Detail_Js.approveOrReject('index.php?module=" . $this->getModule()->getName() .
					"&action=ApproveOrReject&record=$recordId&source_module=Guard','Rejected')",
				'linkicon' => ''
			);
			$linkModelList['DETAILVIEWBASIC'][] = Vtiger_Link_Model::getInstanceFromValues($rejectLink);
		}
